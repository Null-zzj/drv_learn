...
static irqreturn_t gpio_key_isr(int irq, void *dev_id)
{
	...
	mod_timer(&gpio_desc->key_timer, jiffies + HZ/5);   // 启动启动定时器， 调用定时器的函数 key_timer_expire
	return IRQ_HANDLED;
}
...
static int __init gpio_drv_init(void)
{
	...
	{		
		...
		err = request_irq(gpios[i].irq, gpio_key_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING  | IRQF_SHARED, "100ask_gpio_key", &gpios[i]);   // IRQF_SHARED 设置中断共享才能对多个函数使用同一个软中断号
	}
